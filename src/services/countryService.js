import { AlreadyInDbError, InvalidIdError, NotFoundError, ValidationFailedError } from "../errors/errors";
...
export const findCountryById = async (id) => {
  if (validator.isMongoId(id)) {
    try {
      const countryById = await countryModel.findOne({ _id: id }).populate("cities").populate("region");
      return countryById;
    } catch (error) {
      console.error(error);
      throw error;
